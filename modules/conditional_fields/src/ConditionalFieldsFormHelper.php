...
class ConditionalFieldsFormHelper {
  ...
  public static function afterBuild($form, FormStateInterface &$form_state) {
    ...
    foreach ($form['#conditional_fields'] as $dependent => $dependent_info) {
      ...
      if (in_array('subform', $dependent_info['field_parents'], TRUE)) {
        //NestedArray::getValue($product_selection, array('grandparent' => array('parent')));
        //$x = [$dependent_info['field_parents'][0] => $dependent_info['field_parents'][1][$dependent_info['field_parents'][2]]];
        //$dependent_location = array_slice($dependent_info['field_parents'], 0, -3);
        $dependent_location = $dependent_info['field_parents'];
      }
      //kint($dependent_location);
